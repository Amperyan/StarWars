...
public class PeaksReader {
	Vector<Peaks> listOfPeaks = new Vector<Peaks>(50, 50);
			...
			while (scanner.hasNextLine()) {
				...
				try {
					...
					Peaks peak = new Peaks(Double.parseDouble(readTheta),
							Double.parseDouble(readIntensity));
	...
	public void sortMax() {
		...
		for (int j = 0; j < this.listOfPeaks.size(); j++) {
			Peaks max = new Peaks(get(j).theta, get(j).intensity);
			int maxIndex = j;
	...
	public void printList() {
		for (int i = 0; i < this.listOfPeaks.size(); i++) {
			System.out.println(get(i).theta + "  "
					+ get(i).intensity);
		}
		System.out.println("---------------------------");
		System.out.println();
